...
Eight consecutive seeds (0–7), fixed 3125-step clock, validation every 10 steps; the Track 3 statistic
...
- `train_gpt_muonh_maxentslop.py` — PR #359's script with the first-moment change (and a `--seed` that seeds parameter initialization; PR #359's script seeds nothing, so its paired cells were also run through the same seeding line).
